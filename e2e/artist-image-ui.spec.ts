...
// ---------------------------------------------------------------------------
// 5.3: My Artists Grid Tile Thumbnail
// ---------------------------------------------------------------------------

test.describe('My artists grid tile thumbnail', () => {
	test.beforeEach(async ({ page }) => {
		await mockRpcRoutes(page)
		await mockLastFmApi(page)
		await mockOidcDiscovery(page)
	...
	test('5.3a: grid tile shows thumbnail background when thumbUrl exists', async ({
		page,
	}) => {
		...
		await expect(artistRows.first()).toBeVisible({ timeout: 15_000 })

		// Switch to grid view
		const toggleBtn = page.locator('.toggle-view-btn')
		...
		// YOASOBI tile should have data-thumb attribute (has thumbUrl)
		const thumbTile = page.locator('.grid-tile[data-thumb]').first()
		await expect(thumbTile).toBeVisible()

		// Verify the CSS custom property is set with the thumb URL
		const style = await thumbTile.getAttribute('style')
		expect(style).toContain(FANART_THUMB_URL)
	})

	test('5.3b: grid tile falls back to gradient when no thumbUrl', async ({
		page,
	}) => {
		await page.goto('http://localhost:9000/my-artists')
		...
		// Vaundy tile should NOT have data-thumb (no thumbUrl)
		const noThumbTile = page.locator('.grid-tile:not([data-thumb])').first()
		await expect(noThumbTile).toBeVisible()
	})
})
