/**
 * Published whenever per-purpose consent state changes (initial hydration
 * excluded). Subscribers react by updating their SDK posture —
 * AnalyticsService swaps PostHog persistence/identification mode on every
 * grant/revoke so the SDK stays aligned with the user's latest choice
 * without the route VMs needing to know how each SDK is configured.
 *
 ...
 * consent state directly, and replaying that read as an event would
