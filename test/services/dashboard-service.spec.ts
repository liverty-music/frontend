...
describe('DashboardService', () => {
	...
	it('should load and group concerts for multiple artists', async () => {
		// Arrange
		const artist1 = { artist: { id: { value: 'artist-1' }, name: { value: 'Artist One' } }, passionLevel: 0 }
		const artist2 = { artist: { id: { value: 'artist-2' }, name: { value: 'Artist Two' } }, passionLevel: 0 }
	...
	it('should format concert times correctly', async () => {
		// Arrange
		const artist = { artist: { id: { value: 'artist-1' }, name: { value: 'Artist' } }, passionLevel: 0 }
		mockArtistService.getClient!().listFollowed = vi.fn().mockResolvedValue({
			artists: [artist],
		})
